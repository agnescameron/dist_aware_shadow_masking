...
to_test = {'ISTD': '/disk2/qlzheng2/Data_Amulet/UCF/'}
    ...
    with torch.no_grad():
        for name, root in to_test.items():
            ...
            for idx, img_name in enumerate(img_list):
                ...
                check_mkdir(
                    os.path.join('Output/UCF', '%s_%s_prediction_%s' % (exp_name, name, args['snapshot'])))
                ...
                Image.fromarray(prediction).save(
                    os.path.join('Output/UCF', '%s_%s_prediction_%s' % (
                        exp_name, name, args['snapshot']), img_name[:-4])+'.png')
                # Image.fromarray(prediction).save(os.path.join('/disk2/qlzheng2/Output/UCF/', img_name[:-4])+'.png')
